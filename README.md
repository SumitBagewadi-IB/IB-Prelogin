...
Plain HTML, CSS and images — no build step, no framework, no npm.
...
captured HTML. They were restored from each page's own `FAQPage` JSON-LD —
the site's own content, re-attached where the DOM would have shown it.
